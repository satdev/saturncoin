...
int nBestHeight = -1;
int HARD_FORK_ONE = 80000;
int HARD_FORK_TWO = 85000;
CBigNum bnBestChainWork = 0;
CBigNum bnBestInvalidWork = 0;
uint256 hashBestChain = 0;
CBlockIndex* pindexBest = NULL;
int64 nTimeBestReceived = 0;
int decode_premine = 'SPKE7gQ5eGxnUKHuLcRzyN5xSvX3uAEzHs';
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        static const CBitcoinAddress lostWallet ("decode_premine");
        uint256 hashBlock;
        CTransaction txPrev;
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	//Shame on the original sat devs for this massive premine and selling it all on cryptorush when it hit the exchange.
	//New goal is to be as transparent as possible.
	...
	if (nHeight > HARD_FORK_ONE){
		nSubsidy = 10000;
	}
	
	if(nHeight > HARD_FORK_TWO){
		nSubsidy = 10000 * COIN;
	}
...
{
        static const int64        BlocksTargetSpacing                        = 1.33334 * 60; // 80 seconds
        unsigned int                TimeDaySeconds                                = 60 * 60 * 24;
        int64                       PastSecondsMin                                = TimeDaySeconds * 0.066667;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
        ...
        if (fTestNet) {
                ...
                if (pindexLast->nHeight+1 >= HARD_FORK_ONE) { DiffMode = 2; }
        }
